...
def test_expects_json_body(test_client):
    response = test_client.put('/establishments/1234')
    assert response.status_code == 400
    assert "Expects a JSON body." in response.get_json()['message']
...
def test_updating_an_establishment_does_nothing_with_stale_info(test_client, repo):
    response1 = test_client.put('/establishments/1234',
                                json={'dba': 'La Banquisse', 'zipcode': '02093', 'phone': '4384056262',
                                      'inspection_date': '02/01/19'})
...
def test_updating_an_establishment(test_client, repo):
    ...
    response = test_client.put('/establishments/1234',
                               json={'dba': 'Romado', 'phone': '4384056363', 'inspection_date': '01/01/19'})
    assert response.status_code == 200
